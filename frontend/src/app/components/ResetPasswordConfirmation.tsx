...
const ResetPasswordConfirmation = () => {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      alert("La password è stata resettata con successo.");
      router.push("/");
    } catch (err) {
      alert("Si è verificato un errore durante il reset della password.");
    }
  };
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="px-8 py-6 mt-4 text-left bg-white shadow-lg w-1/3">
        <h3 className="text-2xl font-semibold">Imposta Nuova Password</h3>
        <form onSubmit={handleSubmit(onSubmit)} className="mt-4">
          ...
          />
          {errors.password && (
            <span className="text-xs text-red-600">Password richiesta</span>
          )}
